locals {
  ...
  startup_script = templatefile("${path.module}/startup-script.sh.tmpl", {
    SERVER_KEY  = tls_private_key.server_key.private_key_pem
    SERVER_CERT = tls_locally_signed_cert.server_crt.cert_pem
    LB_ROOT     = tls_self_signed_cert.client_root.cert_pem
  })
}
...
# 🔐 Root CA
####################
resource "tls_private_key" "lb_root" {
  algorithm = "RSA"
  rsa_bits  = 4096
}

resource "tls_self_signed_cert" "lb_root" {
  is_ca_certificate     = true
  private_key_pem       = tls_private_key.lb_root.private_key_pem
...
resource "tls_cert_request" "producer_csr" {
  private_key_pem = tls_private_key.producer.private_key_pem

  dns_names = ["san-gxlb.com"]

  subject {
    common_name  = "san-gxlb.com"
    organization = "San Load Balancer"
  }
}

resource "tls_locally_signed_cert" "producer" {
  cert_request_pem      = tls_cert_request.producer_csr.cert_request_pem
  ca_cert_pem           = tls_self_signed_cert.lb_root.cert_pem
  ca_private_key_pem    = tls_private_key.lb_root.private_key_pem
  validity_period_hours = 876000 # 1 year
...
resource "google_compute_ssl_certificate" "producer" {
  ...
  certificate = tls_locally_signed_cert.producer.cert_pem
  lifecycle {
...
resource "google_compute_target_https_proxy" "producer" {
  ...
  server_tls_policy = "projects/${data.google_project.producer.project_id}/locations/global/serverTlsPolicies/${var.sg_prefix}-tls-policy"
...
resource "google_compute_backend_service" "app_backend" {
  ...
  custom_request_headers          = ["Client-Region: {client_region}", "X-Client-Cert-Present:{client_cert_present}", "X-Client-Cert-Chain-Verified:{client_cert_chain_verified}", "X-Client-Cert-Serial-Number:{client_cert_serial_number}", "X-Client-Cert-SPIFFE:{client_cert_spiffe_id}", "X-Client-Cert-URI-SANs:{client_cert_uri_sans}", "X-Client-Cert-DNSName-SANs:{client_cert_dnsname_sans}", "X-Client-Cert-Error:{client_cert_error}", "X-Client-Cert-Valid-Not-Before:{client_cert_valid_not_before}"]
  tls_settings {
    sni = "examplebackeendsvc.com"
    subject_alt_names {
        dns_name = "examplebackeendsvc.com"    
    }
    subject_alt_names {
        dns_name = "api.examplebackeendsvc.com"    
    }
